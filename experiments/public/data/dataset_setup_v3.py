...
def extract_zip_if_missing(archive_path: Path, target_path: Path, dest_dir: Path, lowercase_name: str | None = None) -> None:
    if _done_marker(target_path).exists():
        print(f"[skip] {target_path}")
        return
    if target_path.exists():
...
def prepare_gui_tasks(root: Path) -> None:
    ...
    extract_zip_if_missing(gui_zip, out_root / "GUIAct", out_root, lowercase_name="guiact")

    mind2web_zip = raw_root / "Mind2Web.zip"
    extract_zip_if_missing(mind2web_zip, out_root / "Mind2Web", out_root, lowercase_name="mind2web")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional local path for image-query metadata (for example, a local clone/download of ziyjiang/MMEB_Test_Instruct).",
    )
    parser.add_argument(
        "--image-query-mode",
