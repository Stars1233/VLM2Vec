...
def _load_query_and_corpus(path_info: Tuple[str, Optional[str], Optional[str]]) -> Tuple[datasets.Dataset, datasets.Dataset]:
...
def build_speechcoco_audio_image_dataset(path_info, **kwargs) -> Tuple[datasets.Dataset, Any]:
    ...
    for row in query_dataset:
        ...
        out_query_text.append(build_query_text("SpeechCOCO"))  # NOTE: Comment translated to English.
        out_query_image.append([None])
        out_query_audio.append(audio_obj)  # NOTE: Comment translated to English.
        out_query_audio_path.append(q_path)

        out_cand_text.append([])  # NOTE: Comment translated to English.
