...
from src.model.vlm_backbone.qwen2_5_omni_backbone.qwen2_5_omni_embed import Qwen25OmniEmbedder
...
class MMEBModel(nn.Module):
    ...
    def __init__(
        ...
        # ---- embedding dim for eval pre-allocation ----
        # Some pipelines (eval.py) pre-allocate tensor with model.rep_dim.
        # Initialize with config.hidden_size when available; otherwise lazy-fill on first encode.
        self.rep_dim = getattr(getattr(encoder, "config", None), "hidden_size", None)
        if self.rep_dim is None:
            self.rep_dim = getattr(getattr(encoder, "config", None), "d_model", None)
        if self.rep_dim is None:
            self.rep_dim = 0  # will be set after first encode_input()
    ...
    def _maybe_set_rep_dim(self, reps: torch.Tensor):
        if isinstance(reps, torch.Tensor) and reps.dim() == 2:
            if getattr(self, "rep_dim", 0) in (0, None):
                self.rep_dim = reps.size(-1)
        return reps
    ...
    def encode_input(self, input):
        ...
        # ===== QWEN2.5 OMNI =====
            ...
            # ✅ wrapper 直接吃 collator 输出 dict，内部完成 forward + pooling + normalize
            reps = self.encoder.encode_from_inputs(input)
            return self._maybe_set_rep_dim(reps)
        ...
        # ===== Fallback: all other HF backbones =====
            ...
            reps = self._pooling(hidden_states, attn_mask)
            return self._maybe_set_rep_dim(reps)
    ...
    def build(cls, model_args: ModelArguments, **kwargs):
        ...
        if model_backbone == PHI3V:
            ...
            # ✅ 用 wrapper：full Omni，不提取 thinker
            base_model = Qwen25OmniEmbedder(
                model_name_or_path=model_args.model_name,
                torch_dtype=torch.bfloat16,
                device_map="auto",
            )
            # wrapper 自己会处理 config；这里不需要动 attn_implementation
    ...
    def load(cls, model_args: ModelArguments, is_trainable=True, **kwargs):
        ...
        if model_args.model_backbone in {
            LLAVA_NEXT, QWEN2_VL, QWEN2_5_VL, QWEN2_VL_TOKENSELECTION, QWEN2_5_VL_TOKENSELECTION, E5_V
        }:
            config = AutoConfig.from_pretrained(model_args.model_name, trust_remote_code=True)
            try:
                config._attn_implementation = "flash_attention_2"
